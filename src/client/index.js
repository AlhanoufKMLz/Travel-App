...
import { getDestination, getStartingDate, getReturnDate, calcDuration, updateModal, displaySavedTrip } from '../client/js/app.js';
import { getWeather, getLocation, getImage } from '../client/js/request.js';
...
const performAction = async(e) => {
    ...
    if (trip.destination !== '' && trip.startDate !== '' && trip.endDate !== '' && (trip.startDate < trip.endDate)) {
        const geoLocation = await getLocation(trip.destination);
        ...
        trip.weatherForecast = await getWeather(trip.latitude, trip.longitude);
        trip.image = await getImage(trip.destination, trip.country);
        updateModal(trip);
